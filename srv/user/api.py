...
re_sign, re_digit = re.compile(r'[~!@#$%_;,.]'), re.compile(r'\d')
re_lower, re_upper = re.compile(r'[a-z]'), re.compile(r'[A-Z]')
...
class LoginApi(BaseHandler):
    ...
    def post(self):
        ...
        self.db.user.update_one(dict(_id=u['_id']), {'$set': dict(ip=self.get_ip())})
